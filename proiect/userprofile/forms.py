    ...
    def clean(self):
        ...
        if self.pk:
            if User.objects.filter(email=email_value).exclude(id=self.pk).exists():
                self._errors['email'] = self.error_class(['Emailul deja exista! Te rugam sa adaugi altul!'])
            if User.objects.filter(username=username_value).exclude(id=self.pk).exists():
                self._errors['username'] = self.error_class(['Usernameul deja exista! Te rugam sa adaugi altul!'])
        else:
